  ...
  return (
    <div>
      ...
      {dataList && dataList.map((data) => (
        <h3 key={data._id}>
          <a href={dataPath + "/" + data._id}>{data.title}</a>
        </h3>
      ))}
    </div>
  )
 ...
 */
async function getStaticApiPaths(apiPath) {
 ...
 *
 * @param {String} dataPath The API path
 * @returns {Object} Returns a object from the API
 */
async function getStaticApiProps(apiPath) {
  const response = await fetch("http://localhost:3000" + apiPath)
  const json = await response.json()
  return json.data
}
